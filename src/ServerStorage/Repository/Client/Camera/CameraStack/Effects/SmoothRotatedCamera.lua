...
local SmoothRotatedCamera = {
	ClassName = "SmoothRotatedCamera";

	-- Max/Min aim up and down
	_maxY = math.rad(80);
	_minY = math.rad(-80);
	_zoomGiveY = math.rad(5); -- Only on th
}

function SmoothRotatedCamera.new()
	return setmetatable({
		SpringX = Spring.new(0);
		SpringY = Spring.new(0);
		Speed = 15;
	}, SmoothRotatedCamera)
end
...
function SmoothRotatedCamera:RotateXY(xyRotateVector)
	self.AngleX += xyRotateVector.X
	self.AngleY += xyRotateVector.Y
