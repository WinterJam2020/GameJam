...
local SummedCamera = {ClassName = "SummedCamera"}
...
function SummedCamera.new(cameraA, cameraB)
	return setmetatable({
		_mode = "World";
		_cameraA = assert(cameraA, "No cameraA");
		_cameraB = assert(cameraB, "No cameraB");
	}, SummedCamera)
end
...
function SummedCamera:__index(index)
	...
	elseif SummedCamera[index] then
		...
		error(string.format("[SummedCamera] - %q is not a valid member", tostring(index)))
	end
end
