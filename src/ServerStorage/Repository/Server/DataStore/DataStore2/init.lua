...
function DataStore:_GetRaw()
	...
	self.getRawPromise = self.savingMethod:Get():Then(function(value)
		self.value = value
...
function DataStore:Get(defaultValue, dontAttemptGet)
	...
	if not self.haveValue then
		while not self.haveValue do
			local success, error = self:_GetRaw():Wait()
...
function DataStore:GetTable(default, ...)
	local success, result = self:GetTableAsync(default, ...):Wait()
	if not success then
		error(result)
	end
...
function DataStore:GetTableAsync(default, ...)
	...
	return self:GetAsync(default, ...):Then(function(result)
		local changed = false
		assert(
			type(result) == "table",
		...
		for defaultKey, defaultValue in next, default do
...
function DataStore:IncrementAsync(add, defaultValue)
	return self:GetAsync(defaultValue):Then(function(value)
		return Promise.Promisify(function()
			self:Set(value + add)
		end)()
	end)
...
function DataStore:Save()
	local success, result = self:SaveAsync():Wait()
...
do
	...
	function CombinedDataStore:Set(value, dontCallOnUpdate)
		return self.combinedStore:GetAsync {}:Then(function(tableResult)
...
local DataStoreMetatable = {__index = DataStore}
...
function DataStore2.SaveAll(player)
	if DataStoreCache[player] then
		for _, dataStore in next, DataStoreCache[player] do
...
function DataStore2.new(dataStoreName, player)
	assert(
		type(dataStoreName) == "string" and IsPlayer.Check(player),
		string.format(
	...
	elseif combinedDataStoreInfo[dataStoreName] then
		local dataStore = DataStore2.new(combinedDataStoreInfo[dataStoreName], player)

		dataStore:BeforeSave(function(combinedData)
			for key in next, combinedData do
				if combinedDataStoreInfo[key] then
					local combinedStore = DataStore2.new(key, player)
	...
	playerLeavingConnection = player.AncestryChanged:Connect(function()
		...
		dataStore:SaveAsync():Then(function()
		...
		Promise.Delay(40):Then(function() --Give a long delay for people who haven't figured out the cache :^(
