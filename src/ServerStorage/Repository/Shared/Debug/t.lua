...
--[[**
	...
	@returns True iff the condition is satisfied, false otherwise
**--]]
t.RaycastParams = primitive("RaycastParams")
...
--[[**
	...
	@returns True iff the condition is satisfied, false otherwise
**--]]
t.RaycastResult = primitive("RaycastResult")

-- custom object types

--[[**
	ensures SyncedPoller type

	@param value The value to check against

	@returns True iff the condition is satisfied, false otherwise
**--]]
t.SyncedPoller = primitive("SyncedPoller")

--[[**
	ensures Janitor type

	@param value The value to check against

	@returns True iff the condition is satisfied, false otherwise
**--]]
t.Janitor = primitive("Janitor")
...
function t.valueOf(valueTable)
	local values = {}
	local length = 0
	for _, value in next, valueTable do
		length += 1
		values[length] = value
	end
	...
	**--]]
	function t.strictArray(...)
		...
		return function(value)
			...
			for idx, typeFn in next, valueTypes do
	...
	**--]]
	function t.interface(checkTable)
		...
		return function(value)
			...
			for key, check in next, checkTable do
	...
	**--]]
	function t.children(checkTable)
		...
		return function(value)
			...
			for name, check in next, checkTable do
...
--[[**
	ensures value is an Enumeration of the correct type
	@param EnumerationType The Enumeration type to check with.
	@returns A function that will return true iff the condition is passed
**--]]
function t.enumeration(EnumerationType)
	return function(Value)
		return EnumerationType:Cast(Value)
	end
end

t.enumerationByType = t.enumeration

--[[**
	ensures value is an Enumeration of the correct type
	@param EnumerationName The Enumeration name to check with.
	@returns A function that will return true iff the condition is passed
**--]]
function t.enumerationByString(EnumerationName: string)
	assert(t.string(EnumerationName))
	return function(Value)
		local Success, EnumerationType = pcall(function()
			return Resources:LoadLibrary("Enumerations")[EnumerationName]
		end)

		if not Success then
			return false, EnumerationType
		else
			return EnumerationType:Cast(Value)
		end
	end
