...
local function createRigJoints(parts, rig)
	for _, params in ipairs(rig) do
		local part0Name, part1Name, attachmentName, limits = params[1], params[2], params[3], params[4]
			...
			if a0 and a1 and a0:IsA("Attachment") and a1:IsA("Attachment") then
				...
				if not constraint then
					...
					if RunService:IsClient() then
						warn("[RagdollRigging] - Creating BallSocketConstraint")
					end
				end
...
local function createAdditionalAttachments(parts, attachments)
	for _, attachmentParams in ipairs(attachments) do
		local partName, attachmentName, cframe, baseAttachmentName = attachmentParams[1], attachmentParams[2], attachmentParams[3], attachmentParams[4]
		...
		if part then
			...
			if not attachment or attachment:IsA("Attachment") then
				...
				if not attachment then
					...
					if RunService:IsClient() then
						warn(string.format("[RagdollRigging] - Creating attachment %q", attachmentName))
					end
				else
					attachment.CFrame = cframe
				end
...
local function createNoCollides(parts, noCollides)
	...
	for _, namePair in ipairs(noCollides) do
		local part0Name, part1Name = namePair[1], namePair[2]
...
function RagdollRigging.DisableParticleEmittersAndFadeOutYielding(character, duration)
	...
	while t < duration do
		...
		for part, initialTransparency in next, transparencies do
...
function RagdollRigging.EaseJointFriction(character, duration)
	...
	for _, v in ipairs(descendants) do
		if v:IsA("BallSocketConstraint") and v.Name == BALL_SOCKET_NAME then
			...
			local array = table.create(3, v)
			array[2], array[3] = current, current * scale
			frictionJoints[v] = array
		end
	end
	...
	while t < duration do
		...
		t += dt
		local alpha = math.min(t / duration, 1)
		for _, tuple in next, frictionJoints do
			tuple[1].MaxFrictionTorque = (1 - alpha) * tuple[2] + alpha * tuple[3]
		end
	end
end
