...
local function Bezier(X1, Y1, X2, Y2)
	...
	return function(T)
		if (X1 == Y1 and X2 == Y2) or T == 0 or T == 1 then
			return T
		end
		...
		if InitialSlope >= 0.001 then
			for _ = 0, 3 do
				GuessForT -= (((((1 - 3 * X2 + 3 * X1) * GuessForT + (3 * X2 - 6 * X1)) * GuessForT + 3 * X1) * GuessForT) - T) / (3 * (1 - 3 * X2 + 3 * X1) * GuessForT * GuessForT + 2 * (3 * X2 - 6 * X1) * GuessForT + 3 * X1)
			end
