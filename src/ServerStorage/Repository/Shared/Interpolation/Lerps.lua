...
local function Color3Lerp(c0, c1, t)
	local u0, v0, u1, v1
...
return Table.Lock({
	...
	Region3 = function(start, finish, alpha) -- @author Sharksie
		local imin = Lerp(start.CFrame * (-start.Size / 2), finish.CFrame * (-finish.Size / 2), alpha)
		local imax = Lerp(start.CFrame * ( start.Size / 2), finish.CFrame * ( finish.Size / 2), alpha)
	...
	NumberSequence = function(start, finish, alpha)
		...
		for _, ap in ipairs(start.Keypoints) do
			local closestAbove, closestBelow

			for _, bp in ipairs(finish.Keypoints) do
