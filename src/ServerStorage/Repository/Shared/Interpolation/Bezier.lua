...
function Bezier.new(x1, y1, x2, y2)
	if not (x1 and y1 and x2 and y2) then
		error("Need 4 numbers to construct a Bezier curve")
	end
	...
	return function(t, b, c, d)
		...
		if InitialSlope >= NEWTON_MIN_SLOPE then
			for _ = 1, NEWTON_ITERATIONS do
				...
				GuessForT -= (((g * GuessForT + h) * GuessForT + e) * GuessForT - t) / CurrentSlope
			end
		elseif InitialSlope ~= 0 then
			...
			for _ = 1, SUBDIVISION_MAX_ITERATIONS do
				...
				if BezierCalculation <= SUBDIVISION_PRECISION then
					break
				end
			end
		end
...
return Table.Lock(Bezier, nil, script.Name)