...
local function Qmul(q1,q2) -- Multiply
	...
	array[1], array[2], array[3], array[4] = w1*w2-x1*x2-y1*y2-z1*z2, w1*x2+x1*w2+y1*z2-z1*y2, w1*y2-x1*z2+y1*w2+z1*x2, w1*z2+x1*y2-y1*x2+z1*w2
	return array
end
lib.Qmul = Qmul
...
local function Qinv(q)--Inverse. (q^-1)
	...
	if m>0 then
		local array = table.create(4)
		array[1], array[2], array[3], array[4] = w/m,-x/m,-y/m,-z/m
		return array
	else
...
local function Qpow(q,exponent,choice)
	...
	if vv>0 then
		...
		local m=(w*w+vv)^(exponent/2)
		local theta=exponent*(math.atan2(v,w)+tau*choice)--swag
		local s=m*math.sin(theta)/v

		local array = table.create(4)
		array[1], array[2], array[3], array[4] = m*math.cos(theta),x*s,y*s,z*s
		return array
	else--This is a regular number.  srs.  lol.
		if w<0 then--Quaternions, umad? u dun fool me nub
			local m=(-w)^exponent
			local s=m*math.sin(pi*exponent)*math.sqrt(3)/3
			local array = table.create(4, s)
			array[1] = m*math.cos(pi*exponent)
			return array
		else
			local array = table.create(4, 0)
			array[1] = w^exponent
			return array
		end
	end
		...
		if data then
			...
			if data.autoChoose then
				if q0[1]*q1[1]+q0[2]*q1[2]+q0[3]*q1[3]+q0[4]*q1[4]<0 then
					local old = q1
					q1=table.create(4)
					q1[1], q1[2], q1[3], q1[4] = -old[1],-old[2],-old[3],-old[4]
				end
			end
			local dt,time=1,data.time
			local timeType=type(time)
	...
	__index=function(_,i)
		...
		if data then
			...
			if timeNow>t0 and timeNow<t1 then
				...
				return CFrame.new(p.x,p.y,p.z,QuaternionToCFrame(BezierRotation(data.q0,data.q1,data.w0,data.w1,t)))
			elseif timeNow>=t1 then
				return data.c1
			elseif timeNow<=t0 then--Whatever.
				return data.c0
		...
		if data then
			...
			if timeNow>t0 and timeNow<t1 then
				...
				x0,x1,v0,v1,q0,w0,w1=
					...
					iq
				q1 = table.create(4)
				q1[1], q1[2], q1[3], q1[4] = QuaternionFromCFrame(v)
			elseif timeNow>=t1 then
				x0,x1,v0,v1,q0,w0,w1=data.x1,v.p,iv,iv,data.q1,iq,iq
				q1 = table.create(4)
				q1[1], q1[2], q1[3], q1[4] = QuaternionFromCFrame(v)
			elseif timeNow<=t0 then
				x0,x1,v0,v1,q0,w0,w1=data.x0,v.p,iv,iv,data.q0,iq,iq
				q1 = table.create(4)
				q1[1], q1[2], q1[3], q1[4] = QuaternionFromCFrame(v)
			end
			...
			local c0=CFrame.new(x0.x,x0.y,x0.z,QuaternionToCFrame(q0))
			local dt,time=1,data.time
			local timeType=type(time)
			if timeType=="number" then
				dt=time
		...
		if f then
			if data.tweening then
				if timeNow>t0 and timeNow<t1 then
					local t=(timeNow-t0)/(t1-t0)
					local p=BezierPosition(data.x0,data.x1,data.v0,data.v1,t)
					f(CFrame.new(p.x,p.y,p.z,QuaternionToCFrame(BezierRotation(data.q0,data.q1,data.w0,data.w1,t))))
				elseif timeNow>=t1 then
					f(data.c1)
					data.tweening=false
				end
...
local function newCFrameTween(name,value,updateFunction,time)
	...
	q[1], q[2], q[3], q[4] = QuaternionFromCFrame(value)
