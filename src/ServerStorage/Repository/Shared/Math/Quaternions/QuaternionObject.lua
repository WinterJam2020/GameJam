...
local Quaternion = Resources:LoadLibrary("Quaternion")
...
local function mul(q0,q1)
	...
	if t0=="quaternion" and t1=="quaternion" then
		...
		return new(w0*w1-x0*x1-y0*y1-z0*z1,w0*x1+x0*w1+y0*z1-z0*y1,w0*y1-x0*z1+y0*w1+z0*x1,w0*z1+x0*y1-y0*x1+z0*w1)
	elseif t0=="quaternion" and t1=="number" then
...
local function pow(q0,q1)
	...
	if t0=="quaternion" and t1=="quaternion" then
		...
		if mm>0 then
			if vv>0 then
				local m=math.sqrt(mm)
				local s=math.acos(w0/m)/math.sqrt(vv)
				w0,x0,y0,z0=math.log(m),x0*s,y0*s,z0*s
			else
				w0,x0,y0,z0=math.log(mm)/2,0,0,0
			end
		else
			w0,x0,y0,z0=-math.huge,0,0,0
		end
		local w1,x1,y1,z1=q1.w,q1.x,q1.y,q1.z
		local m=math.exp(w0*w1-x0*x1-y0*y1-z0*z1)
		...
		if vv>0 then
			...
			return new(m*math.cos(v),x*s,y*s,z*s)
		else
			return new(m)
		end
	elseif t0=="quaternion" and t1=="number" then
		...
		if vv>0 then
			local v=math.sqrt(vv)
			local m=(w*w+vv)^(q1/2)
			local theta=q1*math.atan2(v,w)
			local s=m*math.sin(theta)/v
			return new(m*math.cos(theta),x*s,y*s,z*s)
		else
			if w<0 then
				local m=(-w)^q1
	...
	elseif t0=="number" and t1=="quaternion" then
		...
		elseif q0<0 then--Not a good idea to use this.
			local m=(-q0)^w
			local vv=x*x+y*y+z*z
			local mc,ms=m*math.cos(pi*w),m*math.sin(pi*w)
			if vv>0 then
				local v=math.sqrt(vv)
...
local function Qlog(q)
	...
	if mm>0 then
		if vv>0 then
			local m=math.sqrt(mm)
			local s=math.acos(w/m)/math.sqrt(vv)
			return new(math.log(m),x*s,y*s,z*s)
		else
			return new(math.log(mm)/2)--lim v->0 x/v*acos(a/(a*a+v*v)^0.5)=0 when a is positive
		end
	else
...
local function Qsqrt(q)
	...
	local vv=x*x+y*y+z*z
	if vv>0 then
		local m=math.sqrt(w*w+vv)
		local s=math.sqrt((m-w)/2*vv)
		return new(math.sqrt((m+w)/2),x*s,y*s,z*s)
	else
		return new((w*w)^0.25)
	end
end
