...
ShadowImage.ImageColor3 = Color3.new()
...
for _, Elevation in next, ShadowData do
	for _, Data in next, Elevation do
		...
		Data.Position = UDim2.fromScale(0.5, 0.5) + (Data.Offset or UDim2.new())
...
return PseudoInstance:Register("Shadow", {
	Properties = {
		...
		Parent = Typer.AssignSignature(2, Typer.OptionalInstanceWhichIsAGuiObject, function(self, Parent)
			if Parent then
				local function ZIndexChanged()
					...
					for Index = 1, 3 do
						self[ShadowNames[Index]].ZIndex = ParentZIndex - 1
					end
				end
			...
			for Index = 1, 3 do
				self[ShadowNames[Index]].Parent = Parent
			end
		...
		Visible = Typer.AssignSignature(2, Typer.Boolean, function(self, Visible)
			for Index = 1, 3 do
				self[ShadowNames[Index]].Visible = Visible
			end
		...
		ChangeElevation = Typer.AssignSignature(2, Typer.EnumerationOfTypeShadowElevation, Typer.OptionalNumber, function(self, Elevation, TweenTime)
			...
			for Name, Data in next, ShadowData[Elevation.Value] do
				...
				for Property, EndValue in next, Data do
					Tween(
						Object, Property,
						Property == "ImageTransparency" and (1 - EndValue) * self.Transparency + EndValue or EndValue,
						Enumeration.EasingFunction.Deceleration.Value, TweenTime or SHADOW_TWEEN_TIME, true
					)
				end
			end
	...
	Init = function(self)
		for Index = 1, 3 do
			local Name = ShadowNames[Index]
			local Shadow = self.Janitor:Add(ShadowImage:Clone(), "Destroy")
