...
SnackbarImage.ScaleType = Enum.ScaleType.Slice
...
local LocalPlayer, PlayerGui do
	if RunService:IsClient() then
		...
		until LocalPlayer or not Scheduler.Wait2(0.03)

		repeat
			PlayerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui")
		until PlayerGui or not Scheduler.Wait2(0.03)
	end
end
...
return PseudoInstance:Register("Snackbar", {
	...
	Methods = {
		Enter = function(self)
			...
			Tween(SnackbarFrame, "Position", self.EnterPosition, Enumeration.EasingFunction.Deceleration.Value, self.ENTER_TIME, false, function(Completed)
				if Completed == TweenCompleted and Scheduler.Wait(self.DisplayTime) then
		...
		Dismiss = function(self)
			if not self.Dismissed then
				self.Dismissed = true
				local SnackbarFrame = self.Object
				SnackbarFrame.ZIndex -= 1

				Tween(SnackbarFrame, "Position", self.ExitPosition, Enumeration.EasingFunction.Acceleration.Value, self.ENTER_TIME, true, function(Completed)
	...
	Events = table.create(1, "OnAction");
	Internals = {
		...
		AdjustSnackbarSize = function(self)
			local Width = self.ActionButtonWidth + self.TextWidth + 48
	...
	Properties = {
		...
		ActionColor3 = Typer.AssignSignature(2, Typer.Color3, function(self, Color3)
			if self.SnackbarAction.Parent ~= nil then
				self.SnackbarAction.PrimaryColor3 = Color3
				self:Rawset("ActionColor3", Color3)
			end
		end);
	...
	Init = function(self, ...)
		...
		self.SnackbarPosition = Enumeration.SnackbarPosition.Center.Value
