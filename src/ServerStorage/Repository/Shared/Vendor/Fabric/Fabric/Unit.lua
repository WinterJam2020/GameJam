...
local Unit = {ClassName = "Unit"}
Unit.__index = Unit

function Unit:fire(eventName, ...)
	if self:isDestroyed() then
		error(string.format(
			"Cannot fire event %q because this unit is destroyed.",
			tostring(eventName)
		))
	end

	-- call a method of the event name if it exists
	local methodName = "on" .. string.upper(string.sub(eventName, 1, 1)) .. string.sub(eventName, 2)
	if self[methodName] then
		debug.profilebegin(string.format(
			"%s: %s",
			tostring(self),
			tostring(methodName)
		))
		...
		if not success then
			warn(string.format(
				"%q method of %s encounetered an error: %s",
				tostring(methodName),
				tostring(self),
				tostring(errorValue)
			))
		...
		if coroutine.status(thread) ~= "dead" then
			warn(string.format(
				"Yielding in %s of %s is not allowed!",
				tostring(methodName),
				tostring(self)
			))
		end
	...
	for i, callback in ipairs(self._listeners[eventName]) do
		...
		if not success then
			warn(string.format(
				"Event listener of %s for %q encountered an error: %s",
				tostring(self),
				tostring(eventName),
				tostring(errorValue)
			))
...
function Unit:on(eventName, callback)
	if self:isDestroyed() then
		error(string.format(
			"Cannot attach event listener %q because this unit is destroyed.",
			tostring(eventName)
		))
	end
...
function Unit:_runEffect(key)
	...
	if coroutine.status(thread) ~= "dead" then
		warn(string.format(
			"Effect %q of %s yielded! This is very illegal.",
			tostring(key),
			tostring(self)
		))
	end
	...
	if not success then
		warn(string.format(
			"Effect %q of %s encountered an error: %s",
			tostring(key),
			tostring(self),
			tostring(errorValue)
		))
...
function Unit:_changed()
	...
	debug.profilebegin(string.format("%s: reduce", tostring(self)))
...
function Unit:__tostring()
	return string.format(
		"Unit(%s)",
		typeof(self.ref) == "Instance" and string.format("%s, %s", self.name, self.ref:GetFullName()) or self.name
	)
end
