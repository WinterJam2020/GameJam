...
local Fabric = {
	...
	t = Resources:LoadLibrary("t");
	DEBUG = true;
	Heartbeat = RunService.Heartbeat;
	None = Symbol.named("None");
	Unit = setmetatable({}, {
		__index = function(_, key)
			error(string.format("Unit %q is not registered!", key))
		end;
	});
}
...
function Fabric:registerUnitsIn(container)
	for _, object in ipairs(container:GetChildren()) do
		if object:IsA("ModuleScript") then
			if not string.match(object.Name, "%.spec$") then
...
function Fabric:getLoadedUnitByRef(unitResolvable, ref)
	...
	if unit == nil then
		error(string.format(
			"Attempt to get loaded unit %q on %s, but it does not exist.",
	...
	if not (unit._loaded or unit._loading) then
		error(string.format(
			"Attempt to call getLoadedUnitByRef on %q on %s, but it will never be loaded.",
			tostring(unitResolvable),
			tostring(ref)
		))
	end
...
function Fabric:fire(eventName, ...)
	...
	for _, callback in ipairs(self._listeners[eventName]) do
		...
		if not success then
			warn(string.format(
				"Event listener for %s encountered an error: %s",
				tostring(eventName),
				tostring(errorValue)
			))
		end
