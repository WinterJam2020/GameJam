...
local UnitCollection = {ClassName = "UnitCollection"}
...
function UnitCollection:resolveOrError(unitResolvable)
	return self:resolve(unitResolvable) or error(string.format(
		"Cannot resolve unit %s",
...
function UnitCollection:removeAllUnitsWithRef(ref)
	if self._refUnits[ref] then
		for _, unit in pairs(self._refUnits[ref]) do
			unit:fire("destroy")
		end
	end
end
