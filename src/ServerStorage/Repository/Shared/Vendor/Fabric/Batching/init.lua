	...
	interval = function(duration, callback)
		local intervalEvent = SinglePromiseEvent.new(function(fire)
			return function(_, _, isCancelled)
				...
				loop = function()
					return Promise.Try(fire):ThenCall(Promise.Delay, duration):Then(function()
		...
		return {
			...
			callback = function(list)
				...
				while currentTime <= duration do
					...
					local p = Promise.Delay(timePerUpdate):Then(function(timeTaken)
					...
					for _ = 1, updatesToDoNow do
						...
						if nextUnit then
							...
							local ok, errorValue = coroutine.resume(thread, nextUnit)
				...
				if seen < #list then
					for i = seen, #list do
						local thread = coroutine.create(callback)
						local ok, errorValue = coroutine.resume(thread, list[i])
	...
	local function setupBatching(staticUnit)
		...
		if staticUnit.batch then
			...
			staticUnit.effects._batchListener = function(self)
				...
				if unitToListeners[unitName] == nil and type(staticUnit.batch) == "function" then
