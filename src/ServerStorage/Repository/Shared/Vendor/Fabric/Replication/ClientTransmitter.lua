...
local ClientTransmitter = {Remote = {}}
...
function ClientTransmitter.Remote:event(unit, transmitEvent, transmitData)
	...
	local transmitStr = "server" .. string.upper(string.sub(transmitEvent, 1, 1)) .. string.sub(transmitEvent, 2)
	transmitter:fire(
		transmitStr,
		transmitData
	)
...
function ClientTransmitter.Remote:rejectNetworkPrediction(unit)
	self.fabric:debug(string.format("Network prediction rejected for %q", tostring(unit)))
