...
function ServerTransmitter:_getTransmitterFromSerializedUnit(serializedUnit, failMode)
	...
	if not unit then
		self.fabric:debug(string.format(
			"Client wants communicate with unit %q on %q, but that doesn't exist on the server. This could be normal if the attached Instance was removed.",
	...
	if not transmitter then
		self.fabric:debug(string.format("%s does not have a Transmitter attached, but received a message.", unit))
	end
...
function ServerTransmitter.Remote:event(player, transmitter, transmitEvent, transmitData, predictionGUID)
	...
	local transmitStr = "client" .. string.upper(string.sub(transmitEvent, 1, 1)) .. string.sub(transmitEvent, 2)
	transmitter:fire(
		transmitStr,
		player,
		transmitData
