...
function Members:Predict(Key, Value)
	...
	local Array = table.create(2, Key)
	Array[2] = Value
	table.insert(self.PredictionBuffer, Array)
...
function Members:Serialize(AtKey, ForClient)
	...
	local Array = table.create(4, AtKey)
	Array[2], Array[3], Array[4] = self._Class.SerialType, SymbolicValue, self._PreservationId
	return Array
end
			...
			for _, Player in ipairs(Players:GetPlayers()) do
				...
				for _, Serialized in next, Buffer do
					...
					repeat
						...
						local Key, NewSerialType, NewPartialSymbolicValue = NestedSerialized[1], NestedSerialized[2], NestedSerialized[3]
...
function Members:_ApplyUpdate(Buffer, DestroyList, RelocatedList, UpdateList)
	...
	if self.Context.Base == self then
		for Signal in next, UpdateList do
			Signal:Fire(IsLocal)
		end
...
function Members:_BufferRawUpdate(WrappedKey, WrappedValue)
	...
	while Key ~= nil and Base ~= nil do
		...
		local NextBuffer = {}
		local Array = table.create(4, Key)
		Array[2], Array[3], Array[4] = NextBase._class.SerialType, NextBuffer, NextBase._PreservationId
		QualifiedBuffer[#QualifiedBuffer + 1] = Array
...
function Members:Destroy()
	...
	for _, Signal in next, self.ValueWillUpdateSignals do
		Signal:Destroy()
	end

	self.ValueWillUpdateSignals = nil
	for _, Signal in next, self.ValueOnUpdateSignals do
		Signal:Destroy()
	end
...
function Replicant:Constructor(PartialConfig, NewContext)
	...
	self.WillUpdate = FastSignal.new()
	self.OnUpdate = FastSignal.new()
...
function Replicant.Extend()
	local SubclassStatics, SubclassMembers = setmetatable({}, {__index = Replicant}), setmetatable({}, {__index = Members})
