...
function Utility.Serialize(Key, Object)
	...
	elseif TypeOf == "table" then
		...
		if #Object == 0 then
			for Index, Value in next, Object do
				...
				SymbolicValue[Index] = Utility.Serialize(Index, Value)
			end
		else
	...
	elseif TypeOf == "ColorSequence" or TypeOf == "NumberSequence" then
		...
		for Index, Keypoint in ipairs(Object.Keypoints) do
			SymbolicValue[Index] = Utility.Serialize(Index, Keypoint)
		end
	elseif TypeOf == "ColorSequenceKeypoint" then
		SerializedType = TypeOf
		SymbolicValue = {
			Utility.Serialize(1, Object.Time),
			Utility.Serialize(2, Object.Value),
		}
	...
	elseif TypeOf == "NumberSequenceKeypoint" then
		...
		SymbolicValue = {
			Utility.Serialize(1, Object.Time),
			Utility.Serialize(2, Object.Value),
			Utility.Serialize(3, Object.Envelope),
		}
	...
	elseif TypeOf == "Region3" then
		SerializedType = TypeOf
		SymbolicValue = {
			Utility.Serialize(1, Object.CFrame.Position - Object.Size / 2),
			Utility.Serialize(2, Object.CFrame.Position + Object.Size / 2),
		}
	...
	Array[1], Array[2], Array[3] = Key, SerializedType, SymbolicValue
	return Array
end

-- Serialized values should be in the form {key, type, symbolic_value, [preservation_id]}
function Utility.Deserialize(Serialized)
	...
	elseif SerializedType == "Table" then
		...
		for _, SerializedValue in ipairs(SymbolicValue) do
			Table[SerializedValue[1]] = Utility.Deserialize(SerializedValue)
		end
	...
	elseif SerializedType == "ColorSequence" then
		...
		for _, SerializedKeypoint in ipairs(SymbolicValue) do
			Keypoints[SerializedKeypoint[2]] = Utility.Deserialize(SerializedKeypoint)
		end
	...
	elseif SerializedType == "ColorSequenceKeypoint" then
		...
		for _, SerializedArgument in ipairs(SymbolicValue) do
			Arguments[SerializedArgument[2]] = Utility.Deserialize(SerializedArgument)
		end
...
function Utility.Inspect(Table, MaxDepth, CurrentDepth, Key)
	...
	for Index, Value in next, Table do
		if type(Value) == "table" then
			Utility.Inspect(Value, MaxDepth, CurrentDepth + 1)
		else
