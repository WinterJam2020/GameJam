...
local function icoSphere(n)
	...
	for _ = 1, n do
		for i = #indices, 1, -3 do
			...
			FastRemove(indices, i - 1)
			FastRemove(indices, i - 2)
		end
	end

	-- normalize
	for index, vert in ipairs(verts) do
		verts[index] = vert.Unit
	end
...
local function vertShape(cf, size2, array)
	local output = table.create(#array)
	for i, v in ipairs(array) do
		output[i] = cf:PointToWorldSpace(v * size2)
	end
