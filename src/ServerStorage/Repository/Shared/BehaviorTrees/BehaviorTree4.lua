...
-- Enumeration.BehaviorTreeStatus
...
local function ProcessNode(Node, Nodes)
	...
	local Type = assert(Enumeration.NodeType:Cast(Node.Type))
	if Type == Enumeration.NodeType.Task then
		local Run = assert(Node.Parameters.Run, "Can't process tree; task leaf node has no run func parameter")
		local TaskNode = AddNode(Enumeration.NodeType.Task)
	...
	elseif Type == Enumeration.NodeType.Blackboard then
		local BlackboardNode = AddNode(Enumeration.NodeType.Blackboard)
		...
		if ReturnType == "true" then
			BlackboardNode.ReturnType = Enumeration.BlackboardQueryType.True
		elseif ReturnType == "false" then
			BlackboardNode.ReturnType = Enumeration.BlackboardQueryType.False
		elseif ReturnType == "unset" or ReturnType == "nil" then
			BlackboardNode.ReturnType = Enumeration.BlackboardQueryType.Nil
		elseif ReturnType == "set" then
			BlackboardNode.ReturnType = Enumeration.BlackboardQueryType.NotNil
		else
			CompareString = true
	...
	elseif Type == Enumeration.NodeType.AlwaysSucceed then
		...
		if Node.Parameters.Nodes[1] then
			...
			(AddNode(Enumeration.NodeType.Succeed)).OnSuccess = true
		end
	elseif Type == Enumeration.NodeType.AlwaysFail then
		...
		if Node.Parameters.Nodes[1] then
			...
			(AddNode(Enumeration.NodeType.Fail)).OnFail = false
		end
	elseif Type == Enumeration.NodeType.Invert then
	...
	elseif Type == Enumeration.NodeType.Repeat then
		...
		if RepeatCount and RepeatCount > 0 then
			AddNode(Enumeration.NodeType.RepeatStart)

			local RepeatNode, RepeatIndex = AddNode(Enumeration.NodeType.Repeat)
	...
	elseif Type == Enumeration.NodeType.While then
	...
	elseif Type == Enumeration.NodeType.Sequence then
	...
	elseif Type == Enumeration.NodeType.Selector then
	...
	elseif Type == Enumeration.NodeType.Random then
		assert(#Node.Parameters.Nodes >= 1, "Can't process tree; random composite node has no children")

		local RandomNode = AddNode(Enumeration.NodeType.Random)
		RandomNode.Indices = {}
	...
	elseif Type == Enumeration.NodeType.Root then
...
local function AssertWarn(Condition, ErrorMessage)
	if not Condition then
		warn(ErrorMessage or "Assertion failed!")
	end
...
function BehaviorTree:Abort(Object, ...)
	...
	if AssertWarn(Enumeration.NodeType:Cast(Node.Type)) == Enumeration.NodeType.Task then
		if Node.Finish then
			Node.Finish(Object, Enumeration.BehaviorTreeStatus.Fail, ...)
		end
	end
...
function BehaviorTree:Run(Object, ...)
	...
	while Index <= NodeCount do
		...
		local Type = assert(Enumeration.NodeType:Cast(CurrentNode.Type))
		...
		if Type == Enumeration.NodeType.Task then
			...
			if Status == nil then
				...
				Status = Enumeration.BehaviorTreeStatus.Fail
			end

			Status = AssertWarn(Enumeration.BehaviorTreeStatus:Cast(Status))

			if Status == Enumeration.BehaviorTreeStatus.Running then
				self.Paused = true
				break
			elseif Status == Enumeration.BehaviorTreeStatus.Success then
				if CurrentNode.Finish then
					CurrentNode.Finish(Object, Status, ...)
				end

				Index = CurrentNode.OnSuccess
			elseif Status == Enumeration.BehaviorTreeStatus.Fail then
				if CurrentNode.Finish then
					CurrentNode.Finish(Object, Status, ...)
				end
		...
		elseif Type == Enumeration.NodeType.Blackboard then
			...
			if Board then
				...
				local String = assert(Enumeration.BlackboardQueryType:Cast(tostring(Value)))
				local ReturnType = assert(Enumeration.BlackboardQueryType:Cast(CurrentNode.ReturnType))

				if CurrentNode.CompareString then
					Result = String and String == ReturnType
				else
					if ReturnType == Enumeration.BlackboardQueryType.True then
						Result = Value == true
					elseif ReturnType == Enumeration.BlackboardQueryType.False then
						Result = Value == false
					elseif ReturnType == Enumeration.BlackboardQueryType.Nil then
						Result = Value == nil
					elseif ReturnType == Enumeration.BlackboardQueryType.NotNil then
						Result = Value ~= nil
					end
				end
			end
		...
		elseif Type == Enumeration.NodeType.Tree then
			local TreeResult = assert(Enumeration.BehaviorTreeStatus:Cast(CurrentNode.Tree:Run(Object, ...)))

			if TreeResult == Enumeration.BehaviorTreeStatus.Running then
				self.Paused = true
				break
			elseif TreeResult == Enumeration.BehaviorTreeStatus.Success then
				Index = CurrentNode.OnSuccess
			elseif TreeResult == Enumeration.BehaviorTreeStatus.Fail then
				Index = CurrentNode.OnFail
			else
		...
		elseif Type == Enumeration.NodeType.RepeatStart then
			Index += 1
		...
		elseif Type == Enumeration.NodeType.Succeed then -- Hanging succeed node (technically a leaf)
			Index = CurrentNode.OnSuccess
		elseif Type == Enumeration.NodeType.Fail then -- Hanging fail node (technically a leaf)
			Index = CurrentNode.OnFail
		else
			error("bad Node.Type")
		end
	...
	if Index == NodeCount + 1 then
		TreeOutcome = Enumeration.BehaviorTreeStatus.Success
	elseif Index == NodeCount + 2 then
		TreeOutcome = Enumeration.BehaviorTreeStatus.Fail
	else
		TreeOutcome = Enumeration.BehaviorTreeStatus.Running
	end
...
function BehaviorTree.new(Parameters)
	...
	local Nodes = {}
	ProcessNode({
		Type = Enumeration.BehaviorTreeStatus.Root;
		Tree = Tree;
		Parameters = {};
	}, Nodes)
...
function BehaviorTree.Sequence(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Sequence;
...
function BehaviorTree.Selector(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Selector;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Random(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Random;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.While(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.While;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Succeed(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.AlwaysSucceed;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Fail(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.AlwaysFail;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Invert(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Invert;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Repeat(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Repeat;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Task(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Task;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.Tree(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Tree;
		Parameters = Parameters;
	}
end
...
function BehaviorTree.BlackboardQuery(Parameters)
	return {
		Type = Enumeration.BehaviorTreeStatus.Blackboard;
		Parameters = Parameters;
	}
end
